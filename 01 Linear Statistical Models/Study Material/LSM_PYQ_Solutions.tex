...
alternative route given wherever the professor's phrasing admits one. Section~3 is a
coverage audit: which crash course contains the machinery for each question, and which
four results appear \emph{nowhere} except the formula sheet.
...
\textbf{Short answer: the \emph{machinery} for every question is in the crash courses, but
four specific results that were actually asked are not.} They appear only on the formula
sheet (added after reading these papers) and now here.
...
\midrule
2024 & 1 & CC2 \S2.2--2.3 (projection TFAE, $\tr=\rank$) & \textbf{Result missing} \\
2024 & 2 & CC2 \S4--5 (estimability, G--M); CC3 \S6 (test) & Fully covered \\
2024 & 3 & CC2 \S2.4--2.5 (g-inverses) & \textbf{Result missing} \\
2024 & 4 & CC3 \S2.3 (Frisch--Waugh) & Formula not stated \\
2025 & 1 & CC3 \S7 (3rd fundamental theorem) & Covered (orientation hedged) \\
2025 & 2 & CC2 \S4.2 (the $2\times2$ $\alpha_i+\gamma_j$ example) & Fully covered \\
2025 & 3 & CC3 \S2.3 (Frisch--Waugh) & Worked instance missing \\
2025 & 4 & CC2 \S2.3 (orthogonal projectors) & \textbf{Result missing} \\
\bottomrule
\end{tabular}
\end{center}

\begin{recipe}{The four gaps, all now closed above}
\begin{enumerate}
  \item \textbf{$\Pj+\mathbf{Q}$ is a projector $\iff\Pj\mathbf{Q}=\mathbf{O}$} (2024 Q1).
        Not in any crash course. Solved in \S1.1.
  \item \textbf{The block g-inverse} $G=\begin{bsmallmatrix}A_{11}^{-1}&0\\0&0\end{bsmallmatrix}$
        (2024 Q3). The crash course defines g-inverses and proves invariance, but never this
        construction. Solved in \S1.3.
  \item \textbf{$\mathbf{A}\succeq0$, $I-\mathbf{A}-\mathbf{B}\succeq0$,
        $\mathbf{B}^2=\mathbf{B}$ $\Rightarrow\mathbf{AB}=\mathbf{BA}=\mathbf{O}$}
        (2025 Q4). Not in any crash course. Solved in \S2.4.
  \item \textbf{$\Var(\wh\beta_j)=\sigma^2/d_j$ and the $T_j$ decomposition} (2024 Q4), and
        the \textbf{ANCOVA instance} of Frisch--Waugh with the $S_{zz},S_{zw},\dots$ notation
        (2025 Q3). The general lemma is in CC3 \S2.3, but neither worked case is.
        Solved in \S1.4 and \S2.3.
\end{enumerate}
\textbf{Everything else the two papers asked is fully developed in the crash courses.}
\end{recipe}
